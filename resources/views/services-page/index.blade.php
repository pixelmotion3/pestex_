                    ...
                    success: function(result) {
                        ...
                        JSON.parse(JSON.stringify(sanitizedResult)).forEach((element, index) => {
                            ...
                            let route_update = `{{ route('methods.update', ':id') }}`.replace(':id',
                                element.id);
                            let methods =
                                `<li class="list-group-item d-flex justify-content-between align-items-start"> <div class="ms-2 me-auto"> <div class="fw-bold">Metodo</div> </div> <span class="badge text-bg-primary rounded-pill"> <a data-bs-toggle="modal" data-bs-target="#edit_methodModal${index}">edit</a> </span> <span class="badge text-bg-primary rounded-pill"> <a href="#" class="text-white" onclick="let result = confirm('Tem a certeza que quer apagar o metodo ?');if(result){ event.preventDefault();document.getElementById('delete-form-method').submit();}">Apagar Metodo</a> <form id="delete-form-method" action="${route_destroy}" method="post" style="display: none"> <input type="hidden" name="_method" value="delete" /> @csrf @('post')</form> </span> </li>`;
