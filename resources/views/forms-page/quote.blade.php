									...
									<div style="max-height: 300px;overflow: auto;">
										@isset($quote_forms)
											@foreach ($quote_forms as $quote_form)
												<div class='d-flex'>
													<div class="col-6">
														<p><b>Tipo ambiente:</b> {{$quote_form["customer_type"]}}</p>
														<p><b>Localidade:</b> {{$quote_form["locality"]}}</p>
														<p><b>Nome:</b> {{$quote_form["name"]}}</p>
														<p><b>Email:</b> {{$quote_form["email"]}}</p>
														<p><b>Telefone:</b> {{$quote_form["phone"]?? "Não informado"}}</p>
														<p><b>IP:</b> {{$quote_form["ip"]?? "- - -"}}</p>
														<p><b>Status:</b> <?php echo $quote_form["viewed"] == null ? "<span class='text-danger'>Não visualizazdo</span>" : "<span class='text-success'>Visualizado</span>"?></p>
														<p><b>Data/hora:</b>
															<?php
																$date_new = new DateTime($quote_form['created_at']);
													...
													<div class="col-6">
														<a href="#"
															...
															onclick="
															let result = confirm('Tem a certeza que quer continua ?');
															if(result){
																event.preventDefault();
																document.getElementById('delete-form-{{$quote_form['id']}}').submit();
															}
															">
															Marcar visto
														</a>
													</div>
													<form id="delete-form-{{$quote_form['id']}}"
														action="{{ route('formshandle.destroy', ['contact_form_id' => $quote_form['id'], 'type_form' => "quote_form"]) }}"
